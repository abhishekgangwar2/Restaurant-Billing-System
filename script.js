...
function printBill() {
  ...
  menuItems.forEach((item, index) => {
    ...
    if (qty > 0) {
      ...
      `;
      orderItems.push({ name: item.name, qty, price: item.price, total: itemTotal });
    }
  });
  ...
  // Save to localStorage (bill history)
  const newBill = {
    customerName,
    contactNumber,
    customerAddress,
  ...
  localStorage.setItem("billHistory", JSON.stringify(savedBills));
  ...
  printWindow.document.write(`
    <html>
      ...
      <body>
        <div class="logo">
          <img src="https://img.freepik.com/premium-vector/restaurant-building-city-background-street_165488-4443.jpg?w=2000">
        ...
        <div class="qr-section">
          <h3>Scan to Pay</h3>
          <img id="qrImage" src="${qrCodeURL}" alt="UPI QR Code" />
          <p><strong>UPI ID:</strong> 933636084@axl</p>
        </div>
...
function filterMenu() {
  ...
  // Filter menuItems by name matching the query
  const filteredItems = menuItems.filter(item =>
    item.name.toLowerCase().includes(query)
  );

  // Reload the menu container with filtered items
  loadMenu(filteredItems);
}
